...
#    （試過，畫面上只剩標題列）。
# 🔴 0 和 8 反過來要白：這兩格在 TUI 裡幾乎只當底色用（Claude Code 的訊息底、
#    側欄、記憶區塊都在這兩格）。留黑的話白底上就是一塊實心黑：字看不見，而且
#    大面積黑正是這個 repo 在消滅的累積來源。設成白＝「不上底」，跟 diff 的
#    context 行不上底是同一個判斷。15 一樣留白（深色底上的白字，例如反白標頭）。
